...
class SashimiDiffWave(nn.Module):
    ...
    def forward(self, input_data) -> Tensor:
        ...
        """
        x, diffusion_steps = input_data # x is (bs, 1, 16000)
        # diffusion_steps is (bs, 1)
        # internally we must permute
        x = x.permute(0, 2, 1) # (bs, 1, seq_len) -> (bs, seq_len, 1)
        y, _ = self.sashimi((x, diffusion_steps)) # y is (bs, seq_len, 1)
        # we need out (bs, 1, seq_len)
        y = y.permute(0, 2, 1) # (bs, seq_len, 1) -> (bs, 1, seq_len)
        return y
